...
% long hamming window
%
% PSDFreqResolution: The zero-padded frequency resolution of the PSD (given
...
function PeakFitObject = lix_doPeakFit(NBTSignal, fs, FrequencyBandsInput, PSDWindow, PSDFreqResolution, PSDOverlap)
...
if(~exist('PSDFreqResolution','var'))
    ...
    PSDFreqResolution = 2^(nextpow2(1/(PSDFreqResolution/fs))); %because padded resolution is given by SamplingFreq/WindowLength
end
...
% [NBTSignal] = nbt_filter_firHp(NBTSignal,0.5,fs,4); %this high-pass is done to get better a PSD


for ChId=1:size(NBTSignal(:,:),2)
    [p1,f1]=pwelch(NBTSignal(:,ChId),PSDWindow,PSDOverlap,PSDFreqResolution,fs); %2^9
    ...
    %% find beta peak
    % This can be tricky (because of the wide frequency range), so we first estimate the peak location using a
