...
switch sepPlot
  ...
  case 'off'
    roi = eval(band);
    TITLE = {sprintf('%s %s of Adults', cellstrcat(chan, '-'), band),...
             sprintf('%s %s of Adolescents', cellstrcat(chan, '-'), band)};
    f = dsearchn(fre', roi');
    c = find(ismember(chans, chan));
    dataSubset = cellfun(@(x) {squeeze(mean(mean(x(f(1):f(2), :, c, :), 1), 3))}, ...
                         data);
    % loYLIM = floor(min(min(cell2mat(dataSubset)))*0.4);
    % hiYLIM = floor(max(max(cell2mat(dataSubset)))*0.4);
    % YLIM = [loYLIM, hiYLIM];
    ...
    % save image
    outName = strcat('lineSpec_', band, '_', num2str(roi(1)), '-', ...
                     num2str(roi(2)), '_', cellstrcat(chan, '-'));
    print(gcf, '-djpeg', '-cmyk', '-painters', ...
          fullfile(outdir, strcat(outName, '.jpg')));
    print(gcf, '-depsc', '-painters', ...
          fullfile(outdir, strcat(outName, '.eps')));
