...
parfor i = 1:nFile
    ...
    rmComps = union(union(rmEye, rmRv), rmOut);
    goodICs = setdiff([1:nbcomp], rmComps);
    % get lefted percentage of data
    if isempty(tmpEEG.icaact)
       EEG.icaact = eeg_getdatact(tmpEEG, ...
                                  'component', 1:size(tmpEEG.icaweights, 1));
    end
    [pvaf, pvafs, vars] = eeg_pvaf(tmpEEG, goodICs, 'plot', 'off');
    EEG.etc.pvaf = pvaf;
    EEG.etc.pvafs = pvafs;
    EEG.etc.vars = vars;
    % save sets
