...
for i = 1:length(vars)  % loop through the group
    ersp(:,i) = pick_ersp(ersp_data{i}, ersp_times, ersp_freqs, ...
...
fname_ersp = sprintf('ersp-%s_f%i-%i_t%i-%i.csv', ...
...
% write
cell2csv(fullfile(output_dir, fname_ersp), ersp_to_write, ',');